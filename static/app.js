...
function quickAsk(text) {
    ...
    sendMessage();
    /* hide chips after first use to give more room */
...
function formatResponse(text) {
     ...
     * 1. Detect contact details → render as contact block
     * 2. Detect numbered / bullet lists → render as <ul>
     * 3. Detect "key: value" pairs → render as highlighted block
     * 4. Bold **text** and *text*
     * 5. Split paragraphs on double newline
     * 6. Single newlines within a paragraph become spaces
     */
    ...
    sections.forEach(section => {
        ...
        if (/(\+\d[\d\s\-]{7,}|@royaldivine|sales@)/i.test(trimmed)) {
            html += `<div class="resp-contact">${inlineFormat(trimmed)}</div>`;
            return;
        }
        ...
        /* ── plain paragraph ── */
        const para = inlineFormat(lines.join(" "));
        html += `<p>${para}</p>`;
    });
